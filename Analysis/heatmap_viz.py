...
def draw_foot(ax, sensor_avgs, prefix, title, cmap, norm,
              is_left=False, peak_total_N=None, body_weight_N=None):
...
def render(data_path='data/gait_data.csv', output_png='data/heatmap.png',
           body_weight_N=None, peak_force_left_N=None, peak_force_right_N=None):
    df = pd.read_csv(data_path)

    sensor_cols  = [c for c in df.columns if c != 'timestamp_s']
    sensor_avgs  = {col: df[col].mean() for col in sensor_cols}

    # Peak total force per foot — prefer caller-supplied actuals (from DB),
    # fall back to summing the CSV sensor columns
    lf_cols = [c for c in sensor_cols if c.startswith('lf_')]
    rf_cols = [c for c in sensor_cols if c.startswith('rf_')]
    peak_lf = peak_force_left_N  if peak_force_left_N  is not None \
              else (float(df[lf_cols].sum(axis=1).max()) if lf_cols else None)
    peak_rf = peak_force_right_N if peak_force_right_N is not None \
              else (float(df[rf_cols].sum(axis=1).max()) if rf_cols else None)

    # Estimate body weight from dominant foot if not supplied
    # (dominant foot peak ≈ 51 % of BW in this insole model)
    if body_weight_N is None and peak_lf is not None and peak_rf is not None:
        body_weight_N = max(peak_lf, peak_rf) / 0.51
    ...
    draw_foot(axes[0], sensor_avgs, 'lf',
              ...
              peak_total_N=peak_lf, body_weight_N=body_weight_N)
    draw_foot(axes[1], sensor_avgs, 'rf',
              'Right Foot',
              cmap, norm, is_left=False,
              peak_total_N=peak_rf, body_weight_N=body_weight_N)
